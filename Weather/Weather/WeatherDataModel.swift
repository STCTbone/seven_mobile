...
//  Weather
//
//  Created by Matthew Rieger on 7/28/16.
...
class WeatherDataModel {
  ...
  func fetchWeatherForLocation(named location: String) {
    let encodedLocation = location.stringByAddingPercentEncodingWithAllowedCharacters(.URLHostAllowedCharacterSet())
    let url = NSURL(string: "http://localhost:3000/weather/location.json?name=\(encodedLocation!)")
    ...
    let task = session.dataTaskWithURL(url!, completionHandler: {
      ...
      if (data != nil) {
        ...
        do { json = try
          NSJSONSerialization.JSONObjectWithData(data!, options: NSJSONReadingOptions()) as? [String:AnyObject]
